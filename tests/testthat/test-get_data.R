...
test_that("get_data connects to the API and returns valid tables with a 
           valid input", {
  vcr::use_cassette(name = "chl_arg_1980", {
    ...
    test_data <- get_data(years = 1980, reporter = "chl", table = "yr")
    expect_is(test_data, "tbl")
    expect_is(test_data, "data.frame")
    expect_output(str(test_data), "21 variables")
    
    # Commodity trade at aggregated level (1980)
    test_data <- get_data(years = 1980, table = "yc")
    expect_is(test_data, "tbl")
    expect_is(test_data, "data.frame")
...
test_that("get_data connects to the API and returns an error after invalid 
           input", {
  # Bilateral trade ABC-CDE fake ISO codes (1980) - Error message
  expect_error(
    get_data(years = 1980, reporter = "abc", partner = "cde"),
    "'arg' should be one of"
  )
             
  # Bilateral trade ABC-CDE fake ISO codes (1980) - Error message
  expect_error(
    get_data(years = 1776, reporter = "usa", partner = "all"),
    "years 1962-2016."
  )
  
  # Bilateral trade Chile-Argentina with fake table (1980) - Error message
  expect_error(
    get_data(years = 1980, reporter = "chl", partner = "arg", table = "abc"),
