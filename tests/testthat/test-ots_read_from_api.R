...
test_that("ots_read_from_api connects to the API and returns valid tables with a 
           valid input", {
  vcr::use_cassette(name = "chl_arg_1980", {
    # Mock countries test inside ots_read_from_api
    ...
    expect_is(test_data, "data.frame")
    expect_output(str(test_data), "13 variables")
    ...
    test_data <- ots_read_from_api(years = 1980, reporter = "chl", table = "yr")
    expect_is(test_data, "data.frame")
    expect_output(str(test_data), "20 variables")

    # Commodity trade at aggregated level (1980)
    test_data <- ots_read_from_api(years = 1980, table = "yc")
    expect_is(test_data, "data.frame")
    expect_output(str(test_data), "21 variables")
  })
})

test_that("ots_read_from_api connects to the API and returns an error after invalid 
           input", {
  # Bilateral trade ABC-CDE fake ISO codes (1980) - Error message
  expect_error(
    ots_read_from_api(years = 1980, reporter = "abc", partner = "cde"),
    "try again"
  )

  # Bilateral trade ABC-CDE fake ISO codes (1980) - Error message
  expect_error(
    ots_read_from_api(years = 1776, reporter = "usa", partner = "all"),
    "try again"
  )

  # Bilateral trade Chile-Argentina with fake table (1980) - Error message
  expect_error(
    ots_read_from_api(years = 1980, reporter = "chl", partner = "arg", table = "abc"),
    "try again"
  )
})
