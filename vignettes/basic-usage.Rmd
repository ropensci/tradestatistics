...
* `group_code`: International categorization of product grops defined after product ID
* `group_name`: English name corresponding to `group_code`
...
### Bilateral trade at group level (Year - Reporter - Partner - Group Code)
...
If we want filtering by group, there are different options:
```{r yrpg3, eval = T}
...
`yrpg` table returns some fields that deserve an explanation:
